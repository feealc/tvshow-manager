...
class TVShow(TVShowBase):
    ...
    def __str__(self) -> str:
        return f'id [{self.id}] Id TMDB [{self.id_tmdb}] Temp [{self.number_of_seasons}] ' \
    ...
    def to_tuple(self):
        array = [self.id, self.id_tmdb, self.name, self.number_of_seasons, self.eu, self.pai]
        return tuple(array)

    def get_url_tmdb(self):
        return f'https://www.themoviedb.org/tv/{self.id_tmdb}'
